...
- Split auth pages (21 Aug 2026): `/login`, `/signup`, `/forgot-password`, and `/reset-password` use a two-column layout (`AuthSplitLayout`). Large screens show a teal-mist panel with logo, command-centre line art, headline, and four product points (`authShowcase` in `frontend/src/content/product.ts`); the form sits on the right. Narrow screens show the logo and form only. Copy does not invent customer counts.
