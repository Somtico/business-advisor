...
- Analysis loading states (14 Aug 2026): `AnalysisProgress` component (staged step list with spinner/checkmarks, cosmetic pacing, unmounts when real results land) + `SkeletonCard` pulse placeholders. Pricing Advisor shows the six datasets being checked plus skeleton cards; Ask Nonso shows a five-step reasoning progression while a question is in flight.
- Help & FAQ page (14 Aug 2026): `/app/help` ("Help & FAQ" nav item) opens with a "Meet Nonso" introduction, then accordion FAQs across About Nonso, Pricing Advisor, Impact Ledger, and Data & Privacy — covering the no-guessing rules, verification tiers, margin configuration, and the advice disclaimer.
