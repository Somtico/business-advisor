...
- Nonso branding (14 Aug 2026): the AI is named **Nonso** in all user-facing copy — nav ("Ask Nonso"), chat page, Pricing Advisor, Action Centre messages, Command Centre "Nonso's Impact" card, weekly brief, and the system prompt (the model refers to itself as Nonso). The first mention on public and signed-in surfaces names it as the AI advisor and states it is software, not a person (`AI_NAME_INTRO` in `frontend/src/content/product.ts`; AppShell, onboarding, Command Centre, Enrolment Advisor, Help, FAQ "Who is Nonso?"). The product name is **AI Business Advisor**.
