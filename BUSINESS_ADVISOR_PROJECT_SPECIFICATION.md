...
- Terms of Service (16 Aug 2026, version `2026-08-16.1`): `/terms` public page for Somtico Technologies Inc. (operating name Somtico Tech) — information-not-advice, assumption of risk, AI limitations (defines Advisor as the AI advisor feature; software, not a person), data responsibility, fees/renewals/no refunds, acceptable use, IP (including Somtico-owned models trained on opted-in privacy-safe outcomes), Privacy Policy incorporated by reference (section 11), warranty disclaimer, liability cap (12 months' fees, no consequential damages), indemnification, termination + 30-day export, Saskatchewan governing law/venue, class-action waiver, 1-year claim limit. Section 7 covers Customer Data licence (no Somtico ownership claim), ordinary internal telemetry, optional cross-customer learning (distinct from Terms acceptance), and optional peer benchmarking (customer UI not yet launched). Contact: Privacy Officer Somto Ufondu, somto@somticoweb.com, 202B Meadows Blvd., Saskatoon, SK S7V 0E4. Signup requires an explicit checkbox covering Terms and Privacy; `POST /api/auth/register` rejects without `termsAccepted` and `privacyAccepted` (`TERMS_REQUIRED`) and stores `termsAcceptedAt` + `termsVersion` plus `privacyAcceptedAt` + `privacyVersion` on the owner user. Material updates use ≥30 days' notice (`LEGAL_NOTICE_PUBLISHED_AT` / `LEGAL_MATERIAL_CHANGE_EFFECTIVE_AT`); existing acceptance rows are never bulk-rewritten. After the effective date, stale versions require explicit re-acceptance via `POST /api/auth/accept-legal` (blocking modal); during the notice window a soft in-app banner offers early accept. `GET /api/auth/me` and login return `legal` status.
- Privacy Policy (16 Aug 2026, version `2026-08-16.1`): separate public page at `/privacy`. Covers Privacy Officer contact/address, account and Customer Data, no sale of personal information, AI providers (Claude preferred → OpenAI; structured tool evidence for inference may include labels already in records such as programme/instructor names or tactic notes; no promise of zero retention solely by using the Service; Customer Data not opted into provider model training), ordinary telemetry vs optional learning, record-specific Enrolment Advisor tactic sharing (`somtico_models_v1`; no contributor key → historical rows generally not deletable by org), optional org-level business-outcome learning (`somtico_models_v2`; pseudonymized contributor key; purpose-specific withdrawal), optional benchmark snapshots (`benchmark_snapshots_v1`; separate purpose; customer comparisons not launched), cross-border processing, children/student records (org responsibility), access/correction, PIPEDA. Optional learning/benchmark consents are not part of mandatory Terms acceptance.
...
- Advisor (AI within the product) with usage metering (Claude preferred → OpenAI fallback; privacy-aware request flags). Local fallback restates leak/impact/pricing verdicts without dumping tool JSON into the chat.
