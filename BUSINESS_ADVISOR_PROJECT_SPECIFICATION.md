...
- Public landing page + brand (14 Aug 2026): `/` is a marketing homepage (logged-in users are sent to `/app`) with Somtico Technologies Inc. attribution, Meet Your Advisor trust points, how-it-works, capabilities, the same product screenshots used on somticoweb.com (`/images/screenshots/`), visitor FAQs, and Start Pilot / Sign In CTAs. Public chrome (`PublicShell`) wraps landing, terms, email verification, invitations, and workspace picker. Footer copy is "{product} is a product of Somtico Technologies Inc." with the company name linked to somticoweb.com (no second company link in the footer nav). Product logo lives at `frontend/public/images/logo/` (`business-advisor-logo.png` full mark, `business-advisor-mark.png` for UI); favicon set is `frontend/public/favicon.ico`, `favicon-32x32.png`, and `apple-touch-icon.png`.
- Split auth pages (21 Aug 2026): `/login`, `/signup`, `/forgot-password`, and `/reset-password` use a two-column layout (`AuthSplitLayout`). Large screens show a teal-mist panel with logo, command-centre line art, headline, and four product points (`authShowcase` in `frontend/src/content/product.ts`); the form sits on the right. The story panel is locked to the viewport height (`dvh`): art and spacing shrink on shorter laptops so the copy stays on screen without scrolling. Narrow screens show the logo and form only. Copy does not invent customer counts. Signup may still scroll on the form column only.
- Marketing screenshots (21 Aug 2026): `backend/scripts/seed-demo-screenshots.ts` seeds (and `--cleanup` deletes) a fictional local org (Northlight Learning Studio) signed in as **John Smith** (`demo@northlight.test`), with programmes, enrolments, wages, sessions, expenses, subscriptions, targets, cash on hand, a PILOT entitlement, and a verified recommendation. The Help Improve Advisor invite is snoozed so it does not cover the shots. `backend/scripts/capture-demo-screenshots.mjs` logs in at three viewports — desktop 1440×900, tablet 768×1024, phone 390×844 — at **2× device scale** (retina PNGs: 2880×1800, 1536×2048, 780×1688) and writes Command Centre, Pricing Advisor, Action Centre, Ask Advisor, and Help & FAQ files (`*-v4.png`, `*-v4-tablet.png`, `*-v4-phone.png`) to the gitignored `.screenshots/` folder, then copies them into `frontend/public/images/screenshots/` and somtico-tech. Galleries serve the original PNGs (`srcset` 2×; Next.js `unoptimized` on somtico-tech) so marketing pages do not recompress the shots. The public gallery and hero default to the visitor's device and expose Phone / Tablet / Desktop tabs; each image is a capture at that size, not a scaled desktop screenshot.
