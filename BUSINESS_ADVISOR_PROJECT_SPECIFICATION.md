...
- Chuk branding (14 Aug 2026): the AI is named **Chuk** in all user-facing copy — nav ("Ask Chuk"), chat page, Pricing Advisor, Action Centre messages, Command Centre "Chuk's Impact" card, weekly brief, and the system prompt (the model refers to itself as Chuk). The first mention on public and signed-in surfaces names it as the AI advisor and states it is software, not a person (`AI_NAME_INTRO` in `frontend/src/content/product.ts`; AppShell, onboarding, Command Centre, Enrolment Advisor, Help, FAQ "Who is Chuk?"). The product name is **AI Business Advisor**.
