...
export async function sendWeeklyExecutiveBrief(organizationId: string) {
  ...
  if (impact.verified.totalCents > 0) {
    impactLines.push(
      `<p>Chuk's verified impact to date: <strong>${dollars(impact.verified.savedCents)} saved · ${dollars(impact.verified.earnedCents)} earned</strong> across ${impact.verified.actionCount} completed action${impact.verified.actionCount === 1 ? '' : 's'}.</p>`
    );
    ...
    impactLines.push(
      '<p>No verified impact from Chuk yet. Complete actions in the Action Centre and confirm their results to build your impact ledger.</p>'
    );
  }
  ...
  const html = `
    ...
    <p style="font-size:14px;color:#555">Generated by AI Business Advisor. Open the app for drill-down evidence.</p>
  ...
  const senderName = process.env.BREVO_SENDER_NAME || 'AI Business Advisor';
